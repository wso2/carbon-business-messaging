...
CREATE TABLE MB_QUEUE_MAPPING (
    QUEUE_ID INT,
    QUEUE_NAME VARCHAR2(512) UNIQUE NOT NULL,
...
CREATE TABLE MB_DTX_PREPARED_ENQUEUE_RECORD (
  ...
  CONSTRAINT FK_MB_DTX_ENQUEUE_RECORD FOREIGN KEY (INTERNAL_XID) REFERENCES MB_DTX_PREPARED_XID (INTERNAL_XID)
  ON DELETE CASCADE
)
/
CREATE TABLE MB_DTX_PREPARED_DEQUEUE_RECORD (
  ...
  QUEUE_NAME VARCHAR2(512) UNIQUE NOT NULL,
  MESSAGE_METADATA RAW(2000) NOT NULL,
  CONSTRAINT PK_MB_DTX_DEQUEUE_RECORD PRIMARY KEY (INTERNAL_XID, MESSAGE_ID, QUEUE_NAME),
  CONSTRAINT FK_MB_DTX_DEQUEUE_RECORD FOREIGN KEY (INTERNAL_XID) REFERENCES MB_DTX_PREPARED_XID (INTERNAL_XID)
  ON DELETE CASCADE
)
/
CREATE TABLE MB_DTX_PREPARED_ENQUEUE_CONTENT (
  ...
  MESSAGE_CONTENT BLOB NOT NULL,
  CONSTRAINT PK_MB_DTX_PREPARED_CONTENT PRIMARY KEY (MESSAGE_ID,CONTENT_OFFSET),
  CONSTRAINT FK_MB_DTX_PREPARED_CONTENT FOREIGN KEY (MESSAGE_ID) REFERENCES MB_DTX_PREPARED_ENQUEUE_RECORD (MESSAGE_ID)
  ON DELETE CASCADE
)
/
CREATE TABLE MB_DTX_PREPARED_DEQUEUE_CONTENT (
  ...
  MESSAGE_CONTENT BLOB NOT NULL,
  CONSTRAINT PK_MB_DTX_PREPARED_CONTENT PRIMARY KEY (MESSAGE_ID,CONTENT_OFFSET),
  CONSTRAINT FK_MB_DTX_PREPARED_CONTENT FOREIGN KEY (MESSAGE_ID) REFERENCES MB_DTX_PREPARED_DEQUEUE_RECORD (MESSAGE_ID)
  ON DELETE CASCADE
)
...
/
CREATE SEQUENCE MB_CLUSTER_EVENT_ID_SEQUENCE START WITH 1 INCREMENT BY 1 CACHE 100 ORDER;
/
...
/
CREATE SEQUENCE MB_MEMBERSHIP_SEQUENCE START WITH 1 INCREMENT BY 1 CACHE 100 ORDER;
/
