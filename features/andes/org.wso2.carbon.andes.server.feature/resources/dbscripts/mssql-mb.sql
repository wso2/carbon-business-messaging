...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[DB0].[MB_DTX_PREPARED_XID]') AND TYPE IN (N'U'))
CREATE TABLE MB_DTX_PREPARED_XID (
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[DB0].[MB_DTX_PREPARED_DEQUEUE_RECORD]') AND TYPE IN (N'U'))
CREATE TABLE MB_DTX_PREPARED_DEQUEUE_RECORD (
                INTERNAL_XID BIGINT NOT NULL,
                MESSAGE_ID BIGINT NOT NULL,
                QUEUE_NAME VARCHAR(512) NOT NULL,
                PRIMARY KEY (MESSAGE_ID),
                FOREIGN KEY (INTERNAL_XID) REFERENCES MB_DTX_PREPARED_XID (INTERNAL_XID)
                ON DELETE CASCADE
);

-- create table dtx prepared enqueue record
IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[DB0].[MB_DTX_PREPARED_ENQUEUE_RECORD]') AND TYPE IN (N'U'))
CREATE TABLE MB_DTX_PREPARED_ENQUEUE_RECORD (
                INTERNAL_XID BIGINT NOT NULL,
                MESSAGE_ID BIGINT NOT NULL,
                MESSAGE_METADATA VARBINARY(MAX) NOT NULL,
                PRIMARY KEY (MESSAGE_ID),
...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[DB0].[MB_DTX_PREPARED_ENQUEUE_CONTENT]') AND TYPE IN (N'U'))
CREATE TABLE MB_DTX_PREPARED_ENQUEUE_CONTENT (
                MESSAGE_ID BIGINT NOT NULL,
                INTERNAL_XID BIGINT NOT NULL,
                CONTENT_OFFSET INTEGER NOT NULL,
                MESSAGE_CONTENT VARBINARY(MAX) NOT NULL,
                PRIMARY KEY (MESSAGE_ID,CONTENT_OFFSET),
                FOREIGN KEY (MESSAGE_ID) REFERENCES MB_DTX_PREPARED_ENQUEUE_RECORD (MESSAGE_ID)
                ON DELETE CASCADE
);

IF NOT EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'[DB0].[MB_DTX_PREPARED_DEQUEUE_CONTENT]') AND TYPE IN (N'U'))
CREATE TABLE MB_DTX_PREPARED_DEQUEUE_CONTENT (
                MESSAGE_ID BIGINT NOT NULL,
                INTERNAL_XID BIGINT NOT NULL,
                CONTENT_OFFSET INTEGER NOT NULL,
                MESSAGE_CONTENT VARBINARY(MAX) NOT NULL,
                PRIMARY KEY (MESSAGE_ID,CONTENT_OFFSET),
                FOREIGN KEY (MESSAGE_ID) REFERENCES MB_DTX_PREPARED_DEQUEUE_RECORD (MESSAGE_ID)
                ON DELETE CASCADE
);
